...
class CNN(nn.Module):
    def __init__(self, dropout_prob, maxpool=True, batch_norm=True, conv_bias=True):
        ...
        self.conv1 = ConvBlock(channels[0], channels[1], kernel_size=3, maxpool=maxpool, batch_norm=batch_norm, dropout=dropout_prob, padding=1)
        self.conv2 = ConvBlock(channels[1], channels[2], kernel_size=3, maxpool=maxpool, batch_norm=batch_norm, dropout=dropout_prob, padding=1)
        self.conv3 = ConvBlock(channels[2], channels[3], kernel_size=3, maxpool=maxpool, batch_norm=batch_norm, dropout=dropout_prob, padding=1)
        
        # MLP block
        self.fc1 = nn.Linear(128 * 6 * 6, fc1_out_dim)
        # For Q2.2
    ...
    def forward(self, x):
        ...
        # Execution of convolutional blocks 
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.conv3(x)
