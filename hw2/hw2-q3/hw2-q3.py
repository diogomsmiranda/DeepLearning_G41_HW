...
def nucleus_sampling(logits, p=0.8):
    ...
    # Sort probabilities and indices in descending order (good for efficiency)
    sorted_probs, sorted_indices = torch.sort(probs, descending=True)
    cumulative_probs = torch.cumsum(sorted_probs, dim=-1)

    # smallest set of tokens whose cumulative probability mass exceeds p
    cutoff_index = torch.searchsorted(cumulative_probs, p)
    top_p_probs = sorted_probs[:cutoff_index + 1]
    top_p_indices = sorted_indices[:cutoff_index + 1]

    # normalizing the top-p probabilities
    top_p_probs = top_p_probs / top_p_probs.sum()

    # sampling from the top-p tokens
    next_token_index = torch.multinomial(top_p_probs, 1)

    return top_p_indices[next_token_index]
