...
class FeedforwardNetwork(nn.Module):
    def __init__(
        ...
        if activation_type == 'relu':
            ...
            raise ValueError(f"Invalid activation type: {activation_type}") # does it ever get to here??
    ...
    def forward(self, x, **kwargs):
        ...
        """
        for layer in self.layers[:-1]: # apply activation & dropout to all layers but the output
...
def train_batch(X, y, model, optimizer, criterion, **kwargs):
    ...
    optimizer.zero_grad() # must do this to clear out the gradients from the last batch
