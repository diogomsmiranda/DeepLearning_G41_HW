...
class LogisticRegression(LinearModel):
    def update_weight(self, x_i, y_i, learning_rate=0.001, l2_penalty=0.0, **kwargs):
        ...
        y_hat_labels = 1 / (1 + np.exp(-self.W.dot(x_i)))
        
        y_true = np.zeros_like(y_hat_labels)
        y_true[y_i] = 1  

        gradient = np.outer((y_hat_labels - y_true), x_i)
...
class MLP(object):
    ...
    def softmax(self, x):
        ...
        return exp_x / (exp_x.sum(axis=0) + 1e-9)
    ...
    def predict(self, X):
        ...
        hidden_input = self.W1.dot(X.T) + self.b1[:, np.newaxis]
        hidden_output = self.relu(hidden_input)
        output_input = self.W2.dot(hidden_output) + self.b2[:, np.newaxis]
    ...
    def train_epoch(self, X, y, learning_rate=0.001, **kwargs):
        ...
        for x_i, y_i in zip(X, y):
            ...
            # Forward pass
            hidden_input = self.W1.dot(x_i) + self.b1
            hidden_output = self.relu(hidden_input)
            output_input = self.W2.dot(hidden_output) + self.b2
            output = self.softmax(output_input)

            # Compute loss
            loss = self.cross_entropy_loss(y_true, output)
            total_loss += loss

            # Backward pass
            output_error = output - y_true
            hidden_error = self.W2.T.dot(output_error) * self.relu_derivative(hidden_input)

            # Update weights and biases
            self.W2 -= learning_rate * np.outer(output_error, hidden_output)
            self.b2 -= learning_rate * output_error
            self.W1 -= learning_rate * np.outer(hidden_error, x_i)
            self.b1 -= learning_rate * hidden_error
